...
private:
    int m_holes;        // number of holes per side
    int m_initialBeans; // number of beans per hole (at the beginning)

    // structure: 2 vectors for the 2 sides, with the 0 index being the pot for each of them
    std::vector<int> north_side; // to store beans on the north side
    std::vector<int> south_side; // to store beans on the south side

    // helper function
    bool invalidHole(int hole) const;
};
